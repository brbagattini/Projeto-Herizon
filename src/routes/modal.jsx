    ...
    >
      <div
        ...
        style={{
          ...
          maxHeight: "85vh",
          overflow: "auto",
          background: "var(--clr-panel, #101010)",
          color: "var(--clr-text, #fff)",
          border: "1px solid var(--clr-border, #565656)",
          borderRadius: "1rem",
      ...
      >
        ...
        {title && (
          <h2 id="modal-title" style={{ margin: "0 0 .75rem", fontWeight: 700 }}>
            {title}
          </h2>
        )}
    ...
    <Modal open={open} onClose={onClose} title={club.nome}>
      ...
      >
        ...
        <ul style={{ margin: 0, paddingLeft: "1rem" }}>
          {club.cidade && (
            <li>
              <strong>Cidade:</strong> {club.cidade}
            </li>
          )}
          {club.estadio && (
            <li>
              <strong>Estádio:</strong> {club.estadio}
            </li>
          )}
          {club.fundacao && (
            <li>
              <strong>Fundação:</strong> {club.fundacao}
            </li>
          )}
          {club.cores && (
            <li>
              <strong>Cores:</strong> {club.cores}
            </li>
          )}
        </ul>
      </div>
    ...
    <Modal open={open} onClose={onClose} title={player.nome}>
      ...
      >
        ...
        <ul style={{ margin: 0, paddingLeft: "1rem" }}>
          ...
          {player.idade && (
            <li>
              <strong>Idade:</strong> {player.idade}
            </li>
          )}
          {player.jogos != null && (
            <li>
              <strong>Jogos:</strong> {player.jogos}
            </li>
          )}
          {player.gols != null && (
            <li>
          ...
          {player.nacionalidade && (
            <li>
              <strong>Nacionalidade:</strong> {player.nacionalidade}
            </li>
          )}
          {player.numero && (
            <li>
              <strong>Número:</strong> {player.numero}
            </li>
          )}
          {player.altura && (
            <li>
              <strong>Altura:</strong> {player.altura}
            </li>
          )}
        </ul>
      </div>
